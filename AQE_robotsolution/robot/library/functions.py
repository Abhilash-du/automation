...
def fmap(f, coll):
    ...
    try:
        return {k: f(v) for k, v in coll.items()}
    except AttributeError:
...
def walk(inner, outer, data):
    ...
    def process_node(inner, k, v):
        if not isinstance(v, collections.Iterable) or isinstance(v, str):
            return inner(k, v)
        if isinstance(v, collections.Sequence):
            rows = tuple(walk(inner, identity, row) for row in v)
            rv = tuple([row for row in rows if row])
        else:
            rv = walk(inner, identity, v)
        return (k, rv) if rv else None
    if isinstance(data, collections.Sequence):
        return outer(tuple([walk(inner, identity, x) for x in data]))
    nodes = tuple([process_node(inner, k_v1[0], k_v1[1]) for k_v1 in iter(data.items())])
    return outer(dict([node for node in nodes if node is not None]))
...
def zipdict(keys, vals):
    ...
    values"""
    return frozendict(*list(zip(keys, vals)))
...
def merge(*fdicts):
    ...
    return merge(first(fdicts), merge(*rest(fdicts)))
...
def walk_replace(smap, data):
    ...
    if isinstance(data, collections.Sequence):
        return tuple([walk_replace(smap, x) for x in data])
    try:
        nodes = tuple([process_node(k_v[0], k_v[1]) for k_v in iter(data.items())])
...
def assoc(fdict, key, val, *kvs):
    keyvals = (key, val) + kvs
    return merge(fdict, dict(list(zip(keyvals[0::2], keyvals[1::2]))))


# not tested with frozen dicts, just regular ones
def dissoc(fdict, key, *ks):
    keys = (key,) + ks
    return {k: v for k, v in fdict.items() if k not in keys}


def hash_map(*keyvals):
    i = iter(keyvals)
    return dict(list(zip(i, i)))
