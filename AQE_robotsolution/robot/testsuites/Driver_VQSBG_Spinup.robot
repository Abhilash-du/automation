...
TC#01_ASDC_Network_Model_Creation
    common find replace    replacing flexware vfc    ${EXECDIR}\\robot\\resources\\SDC\\SDC_DATA.robot     SCN}[${SPACE}a-zA-Z://0-9.-_]*   SCN}${SPACE}${SPACE}${SPACE} VQSBG
    common find replace    replacing flexware vfc    ${EXECDIR}\\robot\\resources\\SDC\\SDC_DATA.robot     NumModels}[${SPACE}a-zA-Z://0-9.-_]*   NumModels}${SPACE}${SPACE}${SPACE} 4
...
TC#03_NW_Spin_Up
  common find replace    replacing flexware vfc    ${EXECDIR}\\robot\\resources\\vid\\VID_DATA.robot     GLOBAL_VNF}[${SPACE}a-zA-Z://0-9.-_]*   GLOBAL_VNF}${SPACE}${SPACE}${SPACE} VQSBG
   ${NumModelsCount}    Evaluate    ${NumModels}+1
